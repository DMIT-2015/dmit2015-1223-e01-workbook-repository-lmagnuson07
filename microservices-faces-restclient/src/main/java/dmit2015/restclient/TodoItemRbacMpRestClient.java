 ...
 */
@RequestScoped
@RegisterRestClient(baseUri = "http://localhost:8181/restapi/TodoItemsDto")
public interface TodoItemRbacMpRestClient {

    @POST
    Response create(TodoItemRbac newTodoItemRbac, @HeaderParam("Authorization") String bearerAuth);

    @GET
    List<TodoItemRbac> findAll();

    @GET
    @Path("/{id}")
    TodoItemRbac findById(@PathParam("id") Long id);

    @PUT
    @Path("/{id}")
    TodoItemRbac update(@PathParam("id") Long id, TodoItemRbac updatedTodoItemRbac, @HeaderParam("Authorization") String bearerAuth);
