 ...
 */
@RequestScoped
@RegisterRestClient(baseUri = "http://localhost:8182/restapi/TodoItemsDto")
public interface TodoItemMultiUserMpRestClient {

    @POST
    Response create(TodoItemMultiUser newTodoItemMultiUser, @HeaderParam("Authorization") String bearerAuth);

    @GET
    List<TodoItemMultiUser> findAll(@HeaderParam("Authorization") String bearerAuth);

    @GET
    @Path("/{id}")
    TodoItemMultiUser findById(@PathParam("id") Long id, @HeaderParam("Authorization") String bearerAuth);

    @PUT
    @Path("/{id}")
    TodoItemMultiUser update(@PathParam("id") Long id, TodoItemMultiUser updatedTodoItemMultiUser, @HeaderParam("Authorization") String bearerAuth);
